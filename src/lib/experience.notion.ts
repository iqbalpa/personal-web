...
const getExperienceContent = React.cache(async (post: any): Promise<IExperience> => {
	...
	const description: string[] = content.map((block: any) => block.bulleted_list_item.rich_text[0].plain_text);
	return {
		id: post.id,
